...
description: Passez en revue les valeurs par défaut et les paramètres disponibles pour les différentes versions de la base de référence de sécurité MDM Windows que vous pouvez gérer avec Microsoft Intune.
author: brenduns
ms.author: brenduns
manager: dougeby
ms.date: 02/07/2020
...
Visualisez les paramètres de la base de référence de sécurité MDM que Microsoft Intune prend en charge pour les appareils qui exécutent Windows 10 ou ultérieur. Les valeurs par défaut des paramètres de cette base de référence représentent la configuration recommandée pour les appareils où elle est applicable. Les valeurs par défaut d’une base de référence peuvent ne pas correspondre aux valeurs par défaut des autres bases de référence de sécurité ou d’autres versions de cette base de référence.

- Pour découvrir plus d’informations sur l’utilisation des bases de référence de sécurité avec Intune et sur la mise à niveau de la version de base de référence dans vos profils de base de référence de sécurité, consultez [Utiliser des bases de référence de sécurité](../security-baselines.md).
- La dernière version de la base de référence est **Base de référence de sécurité MDM pour mai 2019**

Veillez à sélectionner la version de la base de référence que vous voulez voir.
...
> [!NOTE]
> En juin 2019, le modèle *Base de référence de sécurité MDM pour mai 2019* a été publié en disponibilité générale (pas en préversion). Cette version de la base de référence de sécurité remplace la base de référence précédente, qui était la *Base de référence de la sécurité MDM pour octobre 2018*.  Les profils qui ont été créés avant la disponibilité de la base de référence de mai 2019 ne seront pas mis à jour pour refléter les paramètres et les valeurs qui se trouvent dans la version de mai 2019.  Vous ne pouvez pas créer des profils basés sur le modèle en préversion, mais vous pouvez néanmoins modifier et continuer à utiliser les profils que vous avez créés précédemment et qui sont basés sur le modèle en préversion.

Pour découvrir plus d’informations sur les modifications apportées à cette version de la base de référence par rapport à la version précédente, consultez [Ce qui a changé dans le nouveau modèle](#whats-changed-in-the-new-template).

::: zone-end
::: zone pivot="mdm-preview"
**Préversion : base de référence de la sécurité MDM pour octobre 2018** :  
> [!NOTE]
> Il s’agit de la préversion de la base de référence de sécurité MDM, publiée en octobre 2018. Cette base de référence en préversion a été remplacée en juin 2019 par la version du modèle *Base de référence de sécurité MDM pour mai 2019*, qui est en disponibilité générale (pas en préversion). Les profils qui ont été créés avant la disponibilité de la base de référence *Base de référence de sécurité MDM pour mai 2019* ne seront pas mis à jour pour refléter les paramètres et les valeurs de la base de référence de sécurité MDM pour la version de mai 2019. Vous ne pouvez pas créer des profils basés sur le modèle en préversion, mais vous pouvez néanmoins modifier et continuer à utiliser les profils que vous avez créés précédemment et qui sont basés sur le modèle en préversion.
...
- **Empêcher l’utilisateur d’ignorer les erreurs de certificat** :  
...
- **Sécurité basée sur la virtualisation** :  
  **Par défaut** : Activer VBS avec le démarrage sécurisé
...
## <a name="dma-guard"></a>Protection DMA
...
Pour plus d’informations, consultez [2.2.2 FW_PROFILE_TYPE]( https://docs.microsoft.com/openspecs/windows_protocols/ms-fasp/7704e238-174d-4a5e-b809-5f3787dd8acc) dans la documentation des protocoles Windows.

- **Profil de pare-feu Domaine** :  
  Spécifie les profils auxquels appartient la règle : Domaine, Privé ou Public. Cette valeur représente le profil des réseaux qui sont connectés à des domaines.  
...
- **Profil de pare-feu Privé** :  
  Spécifie les profils auxquels appartient la règle : Domaine, Privé ou Public. Cette valeur représente le profil des réseaux privés.  
...
- **Internet Explorer > Zone restreinte : Options d’ouverture de session** :  
  ...
  **Par défaut** : Authentification anonyme
...
- **Scriptlets de la zone Internet d’Internet Explorer** :  
  Ce paramètre de stratégie vous permet de déterminer si l’utilisateur peut exécuter des scriptlets. Si vous activez ce paramètre de stratégie, l’utilisateur peut exécuter des scriptlets. Si vous désactivez ce paramètre de stratégie, l’utilisateur ne peut pas exécuter de scriptlets. Si vous ne configurez pas ce paramètre de stratégie, l’utilisateur peut activer ou désactiver les scriptlets.  
...
- **Sécurité de session minimale pour les serveurs basés sur NTLM SSP** :  
  ...
  **Par défaut** : Exiger NTLM V2 et un chiffrement 128 bits

- **Minutes d’inactivité de l’écran de verrouillage avant activation de l’économiseur d’écran** :  
  Windows détecte l’inactivité d’une session d’ouverture de session et, si la durée d’inactivité dépasse la limite d’inactivité, l’économiseur d’écran s’exécute et verrouille la session.  
  [En savoir plus](https://go.microsoft.com/fwlink/?linkid=2067210)

  **Par défaut** : 15

- **Exiger du client de toujours signer numériquement les communications** :  
...
- **Niveau d'authentification** :  
  ...
  **Par défaut** : Envoyer uniquement les réponses NTLMv2. Refuser LM et NTLM
...
- **Comportement de l’invite d’élévation d’administrateur** :  
  ...
  **Par défaut** : Demande de consentement sur le bureau sécurisé
...
- **Sécurité de session minimale pour les clients basés sur NTLM SSP** :  
  ...
  **Par défaut** : Exiger le chiffrement NTLM V2 128
...
- **Comportement de l’invite d’élévation d’utilisateur standard** :  
  ...
  - *Refuser automatiquement les requêtes d’élévation* : lorsqu’une opération nécessite une élévation de privilège, un message d’erreur d’accès configurable refusé s’affiche. Une entreprise qui fait fonctionner des postes de travail en tant qu’utilisateur standard peut choisir ce paramètre pour réduire les appels au support technique.
  ...
  **Par défaut** : Refuser automatiquement les demandes d’élévation
...
- **Utiliser le mode d’approbation Administrateur** :  
  ...
  - *Oui* : le compte Administrateur intégré utilise le mode d’approbation Administrateur. Par défaut, une invite d’approbation est présentée à l’utilisateur pour toute opération nécessitant une élévation de privilège.
...
- **Détecter les installations d’applications et demander l’élévation** :  
  ...
  - *Désactivé* : les packages d’installation d’application ne sont pas détectés ni invités pour une élévation. Les entreprises qui utilisent des postes de travail d’utilisateur standard et des technologies d’installation déléguée, comme Installation des logiciels de la stratégie de groupe ou Systems Management Server (SMS), doivent désactiver ce paramètre de stratégie. Dans ce cas, la détection du programme d’installation n’est pas nécessaire.
...
- **Assistance à distance sollicitée** :  
  ...
  **Par défaut** : Désactiver l’assistance à distance
  ...
  Quand ce paramètre est défini sur *Activer l’assistance à distance*, configurez les paramètres supplémentaires suivants :

  - **Autorisation d’assistance à distance sollicitée** :  
    **Par défaut** : Afficher
...
- **Activer pour utiliser la détection d’usurpation avancée, si disponible**

  Si la valeur est Oui, les appareils utilisent l’anti-usurpation améliorée, si elle est disponible. Si la valeur est Non, l’anti-usurpation est bloquée. Si le paramètre n’est pas configuré, les configurations effectuées sur le client sont utilisées.  
...
- **Configurer Windows Hello for Business**
  ...
  > [!IMPORTANT]
  > Les options de ce paramètre sont inversées par rapport à leur signification implicite. En cas d’inversion, la valeur *Oui* n’active pas Windows Hello et est traitée comme *Non configurée*. Quand ce paramètre est défini sur *Non configuré*, Windows Hello est activé sur les appareils qui reçoivent cette base de référence.
  >
  > Les descriptions suivantes ont été révisées pour refléter ce comportement. L’inversion des paramètres sera corrigée dans une prochaine mise à jour de cette base de référence de sécurité.

  - Quand la valeur est *Non configuré*, Windows Hello est activé et l’appareil provisionne Windows Hello Entreprise.
  - Quand la valeur est *Oui*, la base de référence n’affecte pas le paramètre de stratégie de l’appareil. Cela signifie que si Windows Hello Entreprise est désactivé sur un appareil, il reste désactivé. S’il est activé, il reste activé.
  <!-- expected behavior 
...
Le modèle *Base de référence de sécurité MDM pour mai 2019* présente les différences suivantes par rapport au modèle*en préversion*.

### <a name="changes-to-the-baseline-settings"></a>Modifications apportées aux paramètres de la base de référence

Les paramètres suivants sont soit :

- *Nouveau* dans cette dernière version de la base de référence.
- *Supprimé* de cette dernière version de la base de référence, mais il était présent dans la version précédente.
- *Modifié* d’une façon ou d’une autre par rapport à ce qu’il était dans la version précédente.

*[Nouveau]* [**Au-dessus du verrouillage**](#above-lock) :
...
*[Nouveau]* [**Device Guard**](#device-guard) :

- **Sécurité basée sur la virtualisation**

*[Nouveau]* [**Protection DMA**](#dma-guard) :
...
- **Internet Explorer > Zone Internet : Demande automatique pour les téléchargements de fichiers** > La valeur par défaut est désormais **Désactivé**. Dans la préversion, ce paramètre était défini sur Activé.
...
*[Nouveau]* [**Windows Hello Entreprise**](#windows-hello-for-business) :
