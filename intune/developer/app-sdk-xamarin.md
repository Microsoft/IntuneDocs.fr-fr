...
Votre application doit définir une classe `Android.App.Application`. Si vous intégrez manuellement GAM, il doit hériter de `MAMApplication`. Vérifiez que votre sous-classe est correctement décorée avec l’attribut `[Application]` et qu’elle se substitue au constructeur `(IntPtr, JniHandleOwnership)`.
