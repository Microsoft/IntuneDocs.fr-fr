...
- **Nom des règles de stratégie de résolution de la table (NRPT)**: règles de stratégie de résolution de noms (NRPT) de table définissent la manière dont DNS résout les noms lorsque cela est connecté au VPN. Une fois que la connexion VPN est établie, choisissez les serveurs DNS qu’utilise la connexion VPN.
