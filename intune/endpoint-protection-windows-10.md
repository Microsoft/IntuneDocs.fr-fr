...
- **Application Guard** : pour activer cette fonctionnalité, utilisez **Activer pour Edge** qui ouvre des sites non approuvés dans un conteneur de navigation virtualisé Hyper-V. **Ne pas configuré** (valeur par défaut) signifie que n’importe quel site (approuvé et non approuvé) s’ouvre sur l’appareil.
...
- **Vérification de liste de révocation de certificats**: choisissez la façon dont l’appareil vérifie la liste de révocation de certificats. Les options disponibles incluent **Désactiver la vérification de la liste de révocation de certificats**, **Échec de vérification de la liste de révocation de certificats sur le certificat révoqué uniquement** et **Échec de vérification de la liste de révocation de certificats pour toute erreur rencontrée**.
...
- **Règles globales de pare-feu Windows Defender de port du magasin local** : choisissez **Activer** pour appliquer des règles globales de pare-feu de port du magasin local à reconnaître et à appliquer. Quand ce paramètre a la valeur **Non configuré** (valeur par défaut), les règles de pare-feu de port globales du magasin local sont ignorées et non appliquées.
- **Règles de pare-feu Windows Defender du magasin local** : choisissez **Activer** pour appliquer des règles de pare-feu du magasin local à reconnaître et à appliquer. Quand ce paramètre a la valeur **Non configuré** (valeur par défaut), les règles de pare-feu du magasin local sont ignorées et non appliquées.
...
- **Avertissement pour tout autre chiffrement de disque** : sélectionnez **Bloquer** pour désactiver l’invite d’avertissement si un autre service de chiffrement de disque se trouve sur l’appareil. **Non configuré** (valeur par défaut) permet l’affichage de l’avertissement.
    - **Permettre aux utilisateurs standard activer le chiffrement lors de l’Azure AD Join**: lorsque vous choisissez **autoriser**, standards utilisateurs/utilisateurs non-administrateurs peuvent activer le chiffrement BitLocker lorsque l’utilisateur est connecté. Ce paramètre s’applique uniquement aux appareils joints à Azure Active Directory. **Non configuré** permet uniquement aux administrateurs d’activer le chiffrement BitLocker sur l’appareil.
...
- **Récupération d’un lecteur fixe** : **Activer** ce paramètre pour contrôler la façon dont les lecteurs fixes protégés par BitLocker récupèrent quand les informations de démarrage nécessaires ne sont pas disponibles. **Non configuré** (valeur par défaut) désactive cette fonctionnalité.
  ...
  - **Options de récupération dans l’Assistant Installation de BitLocker** : définissez ce paramètre sur **Bloquer** pour que les utilisateurs ne puissent pas voir ni changer les options de récupération. Quand ce paramètre a la valeur **Non configuré** (valeur par défaut), les utilisateurs peuvent voir et changer les options de récupération quand ils activent BitLocker.
  - **Enregistrer les informations de récupération BitLocker dans Azure Active Directory** : choisissez **Activer** pour stocker les informations de récupération BitLocker dans Azure Active Directory (Azure AD). Quand ce paramètre a la valeur **Non configuré** (valeur par défaut), les informations de récupération ne sont pas stockées dans Azure AD.
  - **Informations de récupération BitLocker stockées dans Azure Active Directory** : configurez les parties des informations de récupération BitLocker qui sont stockées dans Azure AD. Les options disponibles sont les suivantes :
    - **Sauvegarder les mots de passe et les jeux de clés de récupération**
    - **Sauvegarder les mots de passe de récupération uniquement**
  - **Stocker les informations de récupération dans Azure Active Directory avant l’activation de BitLocker** : **Exiger** la définition de ce paramètre pour empêcher les utilisateurs d’activer BitLocker, sauf si les informations de récupération BitLocker sont correctement stockées dans Azure AD. **Non configuré** (valeur par défaut) permet aux utilisateurs d’activer BitLocker, même si les informations de récupération ne sont pas correctement stockées dans Azure AD.
...
> [!IMPORTANT]
> Pour permettre l’installation correcte et l’exécution d’applications métier de Win32, paramètres anti-programme malveillant doivent exclure les répertoires suivants de l’analyse :<p>
> **Sur X64 ordinateurs clients**:<br>
> *C:\Program fichiers (x86) \Microsoft Intune gestion Extension\Content*<br>
> *C:\windows\IMECache*
>  
> **Sur X86 ordinateurs clients**:<br>
> *C:\Program Files\Microsoft Intune gestion Extension\Content*<br>
> *C:\windows\IMECache*
...
- **Protection réseau**: protège les connexions sortantes à partir de n’importe quelle application vers des domaines ou des adresses IP de faible réputation. L’objectif consiste à protéger les utilisateurs finaux à partir d’applications avec un accès pour les escroqueries par phishing, exploit d’hébergement de sites et des contenus malveillants sur Internet. Il empêche également les navigateurs tiers de se connecter aux sites dangereux.

  Les options disponibles sont les suivantes :

  - **Non configuré** (valeur par défaut) désactive cette fonctionnalité. Les utilisateurs et les applications ne sont pas bloquées de se connecter à des domaines dangereux. Les administrateurs ne peuvent pas voir cette activité dans le centre de sécurité Windows Defender.
  - **Activer** Active la protection du réseau et bloque les utilisateurs et applications de se connecter à des domaines dangereux. Les administrateurs peuvent voir cette activité dans le centre de sécurité Windows Defender.
  - **Audit uniquement**: les utilisateurs et les applications ne sont pas bloquées de se connecter à des domaines dangereux. Les administrateurs peuvent voir cette activité dans le centre de sécurité Windows Defender.

  [Defender/EnableNetworkProtection CSP](https://docs.microsoft.com/windows/client-management/mdm/policy-csp-defender#defender-enablenetworkprotection)

### <a name="exploit-protection"></a>Exploit Protection

Pour utiliser la protection contre le code malveillant, créez un fichier XML qui inclut les paramètres d’atténuation du système et des applications souhaités. Deux options sont disponibles :
...
- **Connexion à distance sans mot de passe** : **Bloquer** autorise seulement des comptes locaux avec des mots de passe vides à se connecter à l’aide du clavier de l’appareil. **Non configuré** (valeur par défaut) autorise les comptes locaux avec des mots de passe vides à se connecter à partir d’emplacements autres que l’appareil physique.
...
- **Minutes d’inactivité de l’écran de veille avant que l’économiseur d’écran s’active** : entrez le nombre maximal de minutes d’inactivité sur l’écran de connexion du bureau interactif avant le démarrage de l’économiseur d’écran.
...
- **Restreindre les connexions RPC à distance au SAM**: la valeur **autoriser** pour refuser des utilisateurs et groupes à partir des appels RPC à distance pour le Gestionnaire de SAM (Security Accounts), qui stocke les comptes d’utilisateur et mots de passe. **Autoriser** également vous permet de modifier la chaîne de la définition du langage SDDL (Security Descriptor) par défaut pour autoriser ou refuser des utilisateurs et groupes pour effectuer ces appels distants explicitement. **Ne pas configuré** (valeur par défaut) utilise le descripteur de sécurité par défaut et peut autoriser les utilisateurs et groupes effectuer des appels RPC à distance au SAM.
...
- **Intégrité UIA sans emplacement sécurisé** : quand ce paramètre est défini sur **Bloquer**, les applications se trouvant dans un emplacement sécurisé dans le système de fichiers s’exécutent uniquement avec l’intégrité UIAccess. **Non configuré** (valeur par défaut) permet aux applications à s’exécuter avec l’intégrité UIAccess, même si elles ne se trouvent pas dans un emplacement sécurisé dans le système de fichiers.
- **Virtualiser les échecs d’écriture de fichiers et de Registre vers les emplacements par utilisateur**: lorsque la valeur **activé**, les applications qui écrivent des données dans des emplacements protégés échouent. Quand la valeur définie est **Non configuré** (par défaut), les échecs d’écriture d’application sont redirigés au moment de l’exécution vers des emplacements définis par l’utilisateur pour le système de fichiers et le registre.
...
- **Invite avec élévation de privilèges pour les installations d’application** : quand la valeur définie est **Activer**, les packages d’installation d’application ne sont ni détectés ni invités pour une élévation. Quand ce paramètre a la valeur **Non configuré** (valeur par défaut), l’utilisateur est invité à entrer un nom d’utilisateur et un mot de base d’administration quand un package d’installation d’application nécessite une élévation de privilège.
...
- **Exécuter tous les administrateurs en mode d’approbation Administrateur** : définissez la valeur **Activer** pour désactiver le mode d’approbation Administrateur et tous les paramètres de stratégie UAC associés. **Non configuré** (valeur par défaut) active le mode d’approbation Administrateur.
